...
![merge(Observable, Observable)](https://resources.baronzhang.com/rxjava/operator/merge/MergeOperator.png)
...
![merge(Observable[])](https://resources.baronzhang.com/rxjava/operator/merge/MergeIOOperator.png)

### StartWith
**`startWith(T)`**用于在源Observable发射的数据前插入数据。使用**`startWith(Iterable<T>)`**我们还可以在源Observable发射的数据前插入Iterable。官方示意图：
![startWith(T) startWith(Iterable<T>)](https://resources.baronzhang.com/rxjava/operator/merge/StartWithOperator.png)

**`startWith(Observable<T>)`**用于在源Observable发射的数据前插入另一个Observable发射的数据（这些数据会被插入到
源Observable发射数据的前面）。官方示意图：
![startWith(Observable<T>)](https://resources.baronzhang.com/rxjava/operator/merge/StartWithOOperator.png)
...
![concat(Observable<? extends T>, Observable<? extends T>)、concat(Observable<？ extends Observable<T>>)](https://resources.baronzhang.com/rxjava/operator/merge/ConcatOperator.png)
...
![zip(Observable, Observable, Func2)](https://resources.baronzhang.com/rxjava/operator/merge/ZipOperator.png)
...
![comnineLatest(Observable, Observable, Func2)](https://resources.baronzhang.com/rxjava/operator/merge/CombineLatestOperator.png)

下面这张图应该更容易理解：
![comnineLatest(Observable, Observable, Func2)](https://resources.baronzhang.com/rxjava/operator/merge/CombineLatest.png)
...
![switchOnNext(Observable<? extends Observable<? extends T>>)](https://resources.baronzhang.com/rxjava/operator/merge/SwitchOnNextOperator.png)
...
再看看下面的图是不是好理解了呢？！
![join(Observable, Func1, Func1, Func2)](https://resources.baronzhang.com/rxjava/operator/merge/JoinOperator.png)
